      ...
      <div style={section}>
        ...
        <div style={row}>
          ...
          <button onClick={refreshPorts} title="Refresh ports"
            style={{ padding: '6px 10px', borderRadius: 5, border: `1px solid ${t.border}`, background: 'transparent', color: t.muted, cursor: 'pointer', fontSize: 13 }}>
            ↺
          </button>
        </div>
        <div style={row}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <div style={{
            ...
            background: connected ? t.success : t.danger, flexShrink: 0,
          }} />
          <span style={{ fontSize: 12, color: t.muted, flex: 1 }}>
            {connected ? `Connected to ${port}` : 'Not connected'}
          </span>
          {connected ? (
            <button onClick={handleDisconnect}
              style={{ padding: '6px 14px', borderRadius: 6, border: `1px solid ${t.danger}`, background: 'transparent', color: t.danger, cursor: 'pointer', fontSize: 13 }}>
              Disconnect
            </button>
          ) : (
            <button onClick={handleConnect}
              style={{ padding: '6px 14px', borderRadius: 6, border: 'none', background: t.accent, color: '#fff', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
              Connect
            </button>
          )}
        </div>
